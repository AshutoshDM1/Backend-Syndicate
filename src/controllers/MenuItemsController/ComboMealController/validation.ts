...
export const createComboMealSchema = z.object({ 
    ...
    rating: z.number().min(0, 'Rating cannot be negative').max(5, 'Rating cannot exceed 5').multipleOf(0.1, 'Rating must have at most 1 decimal place').optional().nullable(),
    prepTime: z.number().int('Preparation time must be an integer').min(0, 'Preparation time cannot be negative').optional().nullable(),
    calories: z.number().int('Calories must be an integer').min(0, 'Calories cannot be negative').optional().nullable(),
});     
