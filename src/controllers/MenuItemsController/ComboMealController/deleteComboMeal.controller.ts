...
export const deleteComboMeal = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  if (existingComboMeal.orderItems.length > 0) {
    throw new ApiError(
      400,
      'Cannot delete combo meal that has been ordered. Consider marking it as unavailable instead.'
    );
  }
  ...
  res.status(200).json(new ApiResponse(200, null, 'Combo meal deleted successfully'));
});
